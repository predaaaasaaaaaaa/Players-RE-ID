...
Keeps what the diagnostic proved (OSNet-AIN backbone,
no HSV, threshold 0.66 from deep-only distribution) and restores
EMA gallery updates from the original best-setup which weights recent
clean frames heavily instead of averaging the full tracklet window
(which pollutes the centroid with pre-occlusion noise).

Features are stored per-component (hsv, deep) and fused via
FeatureExtractor.similarity() — currently W_HSV=0, so only deep
contributes. HSV plumbing is kept for future ablation.
...
Flow per frame:
  ...
  2. Known track IDs → EMA-update gallery ONLY if crop is clean
...
class ReIDMatcher:
    """Manages player identity with EMA-updated gallery."""
